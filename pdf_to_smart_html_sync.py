...
import json
import re
import webbrowser  # <--- הוספנו את הספרייה לפתיחת הדפדפן
...
def parse_pdf_to_leads(pdf_path):
    print(f"קורא את קובץ ה־PDF: {pdf_path}...")
    ...
    for line in lines:
        ...
        is_represented = any(keyword in line.lower() for keyword in ["עו\"ד", "adv", "law", "shibolet", "melamed", "ruchama", "gcblaws"])
        
        leads.append({
            "name": line,
            "hp": "51" + "".join(filter(str.isdigit, line))[:7] if len(line) > 3 else "510000000",
            "quota": "5",
            "phone": "",
            "email": "office@company.co.il" if not is_represented else "lawyer@lawfirm.co.il",
            "source": "מיוצג ע\"י מייצג" if is_represented else "ישיר / חברה",
            "type": "red" if is_represented else "green"
        })
...
<body>
<div class="container">
    <header><h1>📊 ניהול לידים אוטומטי - היתרי עובדים זרים 2026</h1></header>
    <div class="controls">
...
<script>
    ...
    print(f"קובץ ה־HTML נוצר בהצלחה: {html_filename}")

def upload_to_drive(file_path, drive_file_name="leads_dashboard.html"):
    ...
    if files:
        ...
        print("הקובץ עודכן בהצלחה ב־Google Drive!")
    else:
        file_metadata = {'name': drive_file_name, 'mimeType': 'text/html'}
        service.files().create(body=file_metadata, media_body=media, fields='id').execute()
        print("נוצר והועלה קובץ חדש ל־Google Drive!")

if __name__ == "__main__":
    pdf_file = "permit_list.pdf"
    html_output = "index.html"
    
    if os.path.exists(pdf_file):
        extracted_leads = parse_pdf_to_leads(pdf_file)
        generate_html_file(extracted_leads, html_output)
        upload_to_drive(html_output)
        
        # פתיחה אוטומטית של קובץ ה־HTML כדפדפן ויזואלי במחשב
        webbrowser.open('file://' + os.path.realpath(html_output))
    else:
        print(f"שגיאה: קובץ ה־PDF '{pdf_file}' לא נמצא בתיקייה. אנא הכנס אותו.")